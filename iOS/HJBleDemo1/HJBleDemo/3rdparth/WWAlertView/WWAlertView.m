...
@property (nonatomic, copy) WWPopupInputHandler inputHandler;
...
{
    WWAlertViewConfig *config = [WWAlertViewConfig globalConfig];
    NSArray *items =@[
...
- (instancetype) initWithConfirmTitle:(NSString*)title detail:(NSString*)detail sureBlock:(MMPopupBlock)sureBlock cancelBlock:(MMPopupBlock)cancelBlock
{
    ...
    NSArray *items =@[
                      MMItemMake(config.defaultTextCancel, MMItemTypeHighlight, block),
                      MMItemMake(config.defaultTextConfirm, MMItemTypeHighlight, block)
                      ];
...
- (instancetype)initWithTitle:(NSString *)title
             ...
             inputPlaceholder:(NSString *)inputPlaceholder
                 inputHandler:(WWPopupInputHandler)inputHandler
{
    self = [super init];
...
@interface WWAlertViewConfig()

@end

@implementation WWAlertViewConfig

+ (WWAlertViewConfig *)globalConfig
{
    static WWAlertViewConfig *config;
    static dispatch_once_t onceToken;
    
    dispatch_once(&onceToken, ^{
        
        config = [WWAlertViewConfig new];
...
- (instancetype)init
{
    ...
    if ( self )
    {
        ...
        self.itemHighlightColor = MMHexColor(0x1B70FFFF);
